 ...
 * asset having disappeared since it was added) never blocks login — we
 * simply drop that line and move on, matching the guest-cart model where
...
export async function mergeGuestCartIntoServer(): Promise<void> {
  ...
  await Promise.allSettled(lines.map((line) => addToCart({ asset_id: line.asset_id })));
