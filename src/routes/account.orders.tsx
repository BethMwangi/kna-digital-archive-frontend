...
export const Route = createFileRoute("/account/orders")({
  head: () => ({ meta: [{ title: "Order history — KNA account" }] }),
                    ...
                    <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
                      ...
                      <div className="mt-6 space-y-4">
                        <div className="flex items-center justify-between border-b border-border pb-3">
                          ...
                          <span className="font-display text-2xl tabular-nums">{formatKES(o.total)}</span>
                        </div>
                        {o.items.map((it, i) => {
                          const asset = assets.find((a) => a.id === it.assetId) ?? assets[0];
                          return (
                            <div key={i} className="flex gap-3 border-b border-border pb-4 last:border-b-0">
