...
export const Route = createFileRoute("/admin/assets")({
  head: () => ({ meta: [{ title: "Assets — Urithi Admin" }] }),
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            20,847 records catalogued · 384 pending review
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 border-y border-border bg-paper-warm px-4 py-2">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            className="h-7 text-xs text-flag-red border-flag-red/30"
          >
            Delete
          </Button>
        </div>
              ...
              return (
                <TableRow key={a.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={cn("text-[0.65rem] uppercase tracking-wider", statusStyles[st])}
                    >
                      {st}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-sm">
                    {formatKES(a.priceFrom)}
                  </TableCell>
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button size="sm" className="rounded-none bg-ink text-paper hover:bg-ink/90">
          <Plus className="mr-1.5 h-3 w-3" /> New asset
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a record to the archive. Draft is saved automatically.
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 1 && (
          <div className="grid place-items-center border border-dashed border-border bg-paper-warm py-16 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Up to 500 MB · Archival originals recommended
            </p>
        ...
        {step === 2 && (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Label>Title</Label>
              <Input className="mt-1.5" placeholder="President Kenyatta addresses…" />
            </div>
            <div className="sm:col-span-2">
              <Label>Caption</Label>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {["Editorial", "Commercial", "Educational", "Government", "Internal Use"].map(
                  (l) => (
                    <label
                      key={l}
                      className="flex items-center gap-2 border border-border bg-background p-2 text-sm"
                    >
              ...
              {["Web", "Print", "Archive"].map((t) => (
                <div key={t}>
                  ...
                  <Input
                    type="number"
                    className="mt-1.5"
                    defaultValue={t === "Web" ? 1500 : t === "Print" ? 4500 : 12000}
                  />
                </div>
              ))}
            </div>
        ...
        {step === 4 && (
          <div>
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Records set to Review will be visible in the editorial queue for approval by an
              Administrator.
            </p>
          </div>
        )}
